...
const HomePage = () => {
    ...
    const handleOrder = async (product) => {
        ...
        // Check if already in "To Pay" list to avoid confusion
        try {
            const res = await axios.get('/market/my-orders', {
                headers: { Authorization: `Bearer ${token}` }
            });
            const pending = (res.data?.data || []).find(o =>
                ['pending', 'confirmed'].includes(o.status) && o.productId === product.id
            );

            if (pending) {
                const goToBasket = await alerts.confirm(
                    "Déjà dans votre liste",
                    `"${product.name}" est déjà en attente de paiement dans votre panier. Voulez-vous voir votre panier ?`,
                    "Voir mon panier"
                );
                if (goToBasket) {
                    navigate('/my-orders');
                ...
                // If they cancel "Voir mon panier", we still ask if they want to order ANOTHER one
            }
        } catch (e) { console.warn("Check pending failed", e); }
